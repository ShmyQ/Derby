...
exports.logout = function(req, res){
    ...
    g.loggedIn.splice(g.loggedIn.indexOf(username), 1);
}
...
exports.login = function(req, res, done){
    var username = req.body.username;
    var password = req.body.password;
    for(var i = 0; i < g.loggedIn.length; i++){
        if(g.loggedIn[i] === username){
    ...
    g.accountManager.getAccount(username, password, function(err, account){
        if (err && typeof err === 'string')
            ...
            req.session.username = username;
            g.loggedIn[g.loggedIn.length] = username;
            done(null);
        }
    });
...
// Added this for secure updates to an account.
exports.updateAccount = function(req,update,done){
    if (!exports.isLoggedIn(req)){
        done('not logged in', null);
    }
    else {
        var username = req.cookies.username;
        var password = req.cookies.password;
        g.accountManager.updateAccount(username, password, update, done);
    }
}
